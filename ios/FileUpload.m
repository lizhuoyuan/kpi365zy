...
RCT_EXPORT_METHOD(upload:(NSDictionary *)obj callback:(RCTResponseSenderBlock)callback)
{
  ...
  for (NSDictionary *file in files) {
    ...
    if ([filepath hasPrefix:@"assets-library:"]) {
     // NSLog(@"assets-library:");
      //NSArray<NSURL *> *assetUrls = nil;
      NSURL *assetUrl = [[NSURL alloc] initWithString:filepath];
     /* [assetUrls arrayByAddingObject:(NSURL *) assetUrl];
      PHFetchResult<PHAsset *>  *fetchResult = [PHAsset fetchAssetsWithALAssetURLs:( NSArray<NSURL *> *)assetUrls options:(PHFetchOptions *) nil];
       __block NSData * tempData = nil;
      if(fetchResult.count > 0){
        PHAsset *assest = fetchResult[0];
         NSLog(@"fetchResult: %@", fetchResult);
        PHCachingImageManager *imageManager = [[PHCachingImageManager alloc] init];
        [imageManager
          requestImageForAsset:assest
